...
class BinTree
{
    ...
    private TreeNode AddTree(string word, TreeNode node)
    {
        if(node == null)
    ...
    private int SearchWord(TreeNode node,string word)
    {
        if(node == null)
            return 0;
    ...
    private void PrintAlfR(TreeNode node)
    {
        if(node != null)
        {
            PrintAlfR(node.left);
    ...
    private void GetAllWordsR(TreeNode node, List<(string, int)> word)
    {
        if(node != null)
        {
